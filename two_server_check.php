...
function wx_push($card_no,$money,$db,$school_id,$balance)
{
	// return ;
...
function insertDeal($arr,$db,$school_id)
{
	...
	var_dump($newuser_no);
//	var_dump($youxiaoqi);
	if(intval($youxiaoqi) > intval(time())){
		wx_push($arr[0],$amount,$db,$school_id,$balance);
	}
}
...
{
	$stmt = $db->prepare("select * from zf_card_info where user_no = ?");
	$stmt->execute([$arr[0]]);
	$info = $stmt->fetch();
	if($info)
	{
...
{
	$stmt0 = $db->prepare("select * from zf_card_info where user_no = ?");
	$stmt0->execute([$arr[7]]);
	$info = $stmt0->fetch();
	if($info)
	{
