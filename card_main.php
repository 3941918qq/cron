...
function insertDeal($arr, $db, $school_id) {
	...
	$newuser_no = getUserNumber($info['user_no'], $school_id);

	//$ischool_db = sendWeiXin::getDB();
	// TEST IF THE KEY EXIST
	$redis = new redis();
        $result = $redis->connect('127.0.0.1', 6379);
	$redis_set_key = md5($school_id.$newuser_no.trim($arr[11]).strtotime($arr[2]));
	$redis_exist = $redis->sIsMember('card_temp_pool', $redis_set_key); 
	if($redis_exist) return;
	else $redis->sAdd('card_temp_pool', $redis_set_key);
	// 
	$ischool_db = sendWeiXin::getDB();
...
function getUserNumber($user_no, $school_id) {
	if (strlen($user_no) == 7) {
		$newuser_no = "T" . $school_id . $user_no;
	} elseif (strlen($user_no) == 8) {
		$newuser_no = "53" . $user_no;
	} else {
		$newuser_no = "T" . $school_id . $user_no;
	}
	return $newuser_no;
...
function updateCardNo($arr, $db, $school_id) {
	...
	if ($info) {
		...
		try {
			...
			if ($info['phyid'] != $arr[3]) {
				$phyid = sprintf("%010s", $arr[3]);
				$dbh = get_ischooldb();
				//查询卡库epc和电话卡
				$stmt = $dbh->prepare("select * from wp_ischool_kaku where telid = ?");
				$stmt->execute([$phyid]);
				$ret = $stmt->fetch();
				if ($ret) {
					//更新student表的epc号
					$epc_info = $ret['epc'];
					//$tel_info = $ret[''];
					$new_userno = getUserNumber($info['user_no'], $school_id);
					$stmts = $dbh->prepare("select * from wp_ischool_student where stuno2 = ?");
					$stmts->execute([$new_userno]);
					$stu = $stmts->fetch();
					if ($stu) {
						$stmtss = $dbh->prepare("update wp_ischool_student set cardid = ? where stuno2 = ? ");
						$stmtss->execute(array($epc_info, $new_userno));
						//根据学生id更新card表电话卡
						...
						if ($stmtc->rowCount()) {
							...
							$stmtcc->execute(array($phyid, $stu['id']));
						} else {
