...
class sendWeiXin {
	...
	static public function sendCard($post) {
		...
		foreach($ret as $row){ 
			$dealtype = self::loadSchoolConfig($sid, $post['pos_no']);
			if($sid=='56650'){
				$data = self::createTempMsg56650($row['openid'], $tempId, $title, $post['money'],$post['balance'],$post['time'],$dealtype);				
			}else{
				$data = self::createTempMsg($row['openid'], $tempId, $title, $post['money'],$post['balance'],$post['time'],$dealtype);
			}
			$tag = array('type'=>'xyxf=null');						
			Jpush::push($row['openid'],$title,'openid',$tag);
	...
	static function createTempMsg56650($openid, $tempid, $title, $amount, $balance,$timestamp,$dealtype,$url = "") {
		return '{
                       "touser":"' . $openid . '",
                       "template_id":"' . $tempid . '",
                       "url":"' . $url . '",
                       ...
                       "data":{
                          ...
                          "remark":{
                               "value":"正梵智慧校园感谢您的支持。\n\n联系电话:0374-5193099,0371-55030687",
                               "color":"#000000"
                           }
                       }
              }';
